...
from Trajectron.trajec_model.components import *
from Trajectron.trajec_model.model_utils import *
...
class MultimodalGenerativeCVAE(nn.Module):
    def __init__(
        ...
        self.state_length = int(np.sum([len(entity_dims) 
    ...
    def obtain_encoded_tensors(self, mode: ModeKeys, batch: AgentBatch) -> Tuple[torch.Tensor,
                                                                                 torch.Tensor,
                                                                                 torch.Tensor,
                                                                                 torch.Tensor,
                                                                                 torch.Tensor,
        ...
        node_present_state_st = node_history_st[torch.arange(node_history_st.shape[0]), 
                                                node_history_st_len - 1]
        ...
        # Encode History #
        ##################
        node_history_encoded = self.encode_node_history(mode, node_history_st, node_history_st_len)
    ...
    def forward(self, batch: AgentBatch, update_mode: UpdateMode = UpdateMode.BATCH_FROM_PRIOR) -> torch.Tensor:
        return self.train_loss(batch, update_mode=update_mode)

    def train_loss(self, batch: AgentBatch, update_mode: UpdateMode = UpdateMode.BATCH_FROM_PRIOR) -> torch.Tensor:
        """
