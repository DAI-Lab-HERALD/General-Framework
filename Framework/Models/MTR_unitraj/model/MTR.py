...
import MTR_unitraj.model.loss_utils as loss_utils
import MTR_unitraj.model.motion_utils as motion_utils
from MTR_unitraj.model.MTR_utils import PointNetPolylineEncoder, get_batch_offsets, build_mlps
from MTR_unitraj.model.ops.knn import knn_utils
from MTR_unitraj.model.transformer import transformer_decoder_layer, position_encoding_utils, transformer_encoder_layer

Type_array = np.array(['UNSET', 'VEHICLE', 'MOTORCYCLIST', 'CYCLIST', 'PEDESTRIAN'])


class MotionTransformer(nn.Module):
    def __init__(self, config, cluster_path):
        ...
        self.model_cfg.MOTION_DECODER['CENTER_OFFSET_OF_MAP'] = (0, 0)
        self.model_cfg.MOTION_DECODER['OBJECT_TYPE'] = self.model_cfg['object_type']
        self.model_cfg.MOTION_DECODER['CLUSTER_PATH'] = cluster_path
...
class MTRDecoder(nn.Module):
    def __init__(self, in_channels, config):
        ...
        self.cluster_path = self.model_cfg.CLUSTER_PATH
    ...
    def get_motion_query(self, center_objects_type):
        ...
        if self.use_place_holder:
            ...
            Type_strings = Type_array[center_objects_type.cpu().numpy()]
            intention_points = torch.stack([self.intention_points[Type_strings[obj_idx]] for obj_idx in range(num_center_objects)], dim=0).cuda()
