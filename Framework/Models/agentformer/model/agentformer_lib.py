...
def agent_aware_attention(query: Tensor,
    ...
    samples, tgt_len, bsz, embed_dim = query.size()
    assert embed_dim == embed_dim_to_check
    # allow MHA to have different sizes for the feature dimension
    assert key.size(0) == value.size(0) and key.size(1) == value.size(1) and key.size(2) == value.size(2)
    ...
    if attn_mask is not None:
        ...
        if attn_mask.dim() == 3:
            ...
            if list(attn_mask.size()) != [query.size(0), 1, query.size(1), key.size(1)]:
                raise RuntimeError('The size of the 2D attn_mask is not correct.')
        elif attn_mask.dim() == 4:
            if list(attn_mask.size()) != [query.size(0), bsz * num_heads, query.size(1), key.size(1)]:
    ...
    # TODO: has to be reworked later for multiple samples
    ...
    q = q.contiguous().view(samples, tgt_len, bsz * num_heads, head_dim).transpose(1, 2)
    if k is not None:
        k = k.contiguous().view(samples, -1, bsz * num_heads, head_dim).transpose(1, 2)
    if v is not None:
        v = v.contiguous().view(samples, -1, bsz * num_heads, head_dim).transpose(1, 2)
    if in_proj_weight_self is not None:
        q_self = q_self.contiguous().view(samples, tgt_len, bsz * num_heads, head_dim).transpose(1, 2)
        k_self = k_self.contiguous().view(samples, -1, bsz * num_heads, head_dim).transpose(1, 2)
    ...
    if add_zero_attn:
        src_len += 1
        k = torch.cat([k, torch.zeros((k.size(0), k.size(1), 1) + k.size()[3:], dtype=k.dtype, device=k.device)], dim=1)
        v = torch.cat([v, torch.zeros((v.size(0), v.size(1), 1) + v.size()[3:], dtype=v.dtype, device=v.device)], dim=1)
        if attn_mask is not None:
            attn_mask = pad(attn_mask, (0, 1))
        if key_padding_mask is not None:
            key_padding_mask = pad(key_padding_mask, (0, 1))
    ...
    if in_proj_weight_self is not None:
        ...
        attn_weight_self_mask = attn_weight_self_mask.repeat([attn_output_weights.shape[2] // num_agent, attn_output_weights.shape[3] // num_agent]).unsqueeze(0).unsqueeze(0)
        attn_output_weights_self = torch.matmul(q_self, k_self.transpose(2,3))
        ...
        if key_padding_mask is not None:
            attn_output_weights = attn_output_weights.view(samples, bsz, num_heads, tgt_len, src_len)
    ...
    assert list(attn_output.size()) == [samples, bsz * num_heads, tgt_len, head_dim]
    attn_output = attn_output.transpose(1, 2).contiguous().view(samples, tgt_len, bsz, embed_dim)
