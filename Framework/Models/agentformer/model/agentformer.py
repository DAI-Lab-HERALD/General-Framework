...
from agentformer.data.map import GeometricMap
...
class ContextEncoder(nn.Module):
    def __init__(self, cfg, ctx, **kwargs):
        ...
        self.input_type = ctx.get('input_type', 'pos')
    ...
    def forward(self, data):
        ...
        for key in self.input_type:
            if key == 'pos':
                ...
                map_enc = data['map_enc'].unsqueeze(1).repeat((1,data['pre_motion'].shape[1], 1, 1))
...
class FutureDecoder(nn.Module):
    ...
    def decode_traj_ar(self, data, mode, context, pre_motion, pre_vel, pre_motion_scene_norm, z, sample_num, need_weights=False):
        ...
        z_in = z.view(data['batch_size'], dec_in.shape[1], -1, z.shape[-1]) 
        
        if sample_num > z_in.shape[2]:
            z_in = z_in.repeat_interleave(int(sample_num / z_in.shape[2]), dim = 2)
...
class AgentFormer(nn.Module):
    ...
    def set_data(self, data):
        ...
        if self.use_map:
            Maps = np.stack([in_data[i]['scene_map'] for i in range(self.data['batch_size'])])
            self.data['agent_maps'] = torch.from_numpy(Maps).to(device)
