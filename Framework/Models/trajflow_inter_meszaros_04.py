...
class trajflow_inter_meszaros_04(model_template):
    ...
    def train_futureAE(self, T_all):
        ...
        flow_dist_futMdl = TrajFlow_I(pred_steps=self.fut_enc_sz, alpha=self.alpha, beta=self.beta_noise, gamma=self.gamma_noise, 
        ...
        if os.path.isfile(fut_model_file) and not self.data_set.overwrite_results:
            ...
            for epoch in range(num_epochs):
                ...
                while not train_epoch_done:
                    ...
                    scaler = torch.tensor(scipy.stats.truncnorm.rvs((self.s_min-1)/self.sigma, (self.s_max-1)/self.sigma, loc=1, 
                                                                    scale=self.sigma, size=X.shape[0]))
                    scaler = scaler.unsqueeze(1)
                    scaler = scaler.unsqueeze(2)
                    scaler_past = torch.tile(scaler, (1, self.num_timesteps_in, 2)).to(self.device)
                    scaler_future = torch.tile(scaler, (1, self.num_timesteps_out, 2)).to(self.device)
                    
                    past_pos   = X[:,0]
                    future_pos = Y[:,0]
                    
                    
                    mean_pos = torch.mean(torch.concat((past_pos, future_pos), dim=1), dim=1)
                    mean_pos_past = torch.tile(mean_pos.unsqueeze(1), (1,self.num_timesteps_in,1))
                    mean_pos_future = torch.tile(mean_pos.unsqueeze(1), (1,self.num_timesteps_out,1))
                ...
                with torch.no_grad():
                    ...
                    if epoch >= epoch_stop:
                        ...
                        if best_val_step==epoch or epoch == epoch_stop: # Save model if it is the best one
    ...
    def train_flow(self, fut_model, T_all):
        ...
        if os.path.isfile(flow_dist_file) and not self.data_set.overwrite_results:
            ...
            for step in range(steps):
                print('')
                rjust_step = str(step).rjust(len(str(steps)))
                print('Train TrajFlow Normalizing Flows: Epoch ' + rjust_step + '/{}'.format(steps), flush = True)
                ...
                while not train_epoch_done:
                    ...
                    scaler = torch.tensor(scipy.stats.truncnorm.rvs((self.s_min-1)/self.sigma, (self.s_max-1)/self.sigma, loc=1, scale=self.sigma, size=X.shape[0]))
                ...
                with torch.no_grad():
                    ...
                    while not val_epoch_done:
                        ...
                        if img is not None:
                            log_prob = flow_dist.log_prob(out, past_data_val, T, img)
                        else:
                ...
                # Check for convergenc
                step_stop = 50
                if step >= step_stop:
                    best_val_step = np.argmin(val_losses)
                    if step == best_val_step or step == step_stop: # Save model if it is the best one
                ...
                print('Train loss: {:7.5f}; \t Val loss: {:7.5f}'.format(np.mean(losses_epoch), np.mean(val_losses_epoch)))
    ...
    def predict_batch(self, models, test_loader, target_length, batch_sz):
        ...
        while not prediction_done:
            ...
            if img is not None: 
                samples_rel, log_probs = self.flow_dist.sample(self.num_samples_path_pred, past.float(), T, img)
            else:
                samples_rel, log_probs = self.flow_dist.sample(self.num_samples_path_pred, past.float(), T)
