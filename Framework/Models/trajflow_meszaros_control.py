...
import random
import scipy
from TrajFlow.flowModels import TrajFlow_I, Future_Encoder, Future_Decoder_Control, Future_Seq2Seq_Control, Scene_Encoder
import pickle
import os

class trajflow_meszaros_control(model_template):
    '''
    ...
    def train_futureAE(self, T_all):
        ...
        dec = Future_Decoder_Control(actions=2, es=enc_size, hs=enc_size)
    ...
    def train_flow(self, fut_model, T_all):
        ...
        if os.path.isfile(flow_dist_file) and not self.model_overwrite:
            ...
            for step in range(self.flow_epochs):
                ...
                train_epoch_done = False
                while not train_epoch_done:
                    X, Y, T, img, _, _, num_steps, train_epoch_done = self.provide_batch_data('train', 3*self.batch_size, 
                                                                                           val_split_size = 0.1)
