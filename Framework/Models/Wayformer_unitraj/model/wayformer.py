...
from Wayformer_unitraj.model.wayformer_utils import PerceiverEncoder, PerceiverDecoder, TrainableQueryProvider
...
class Wayformer(nn.Module):
    ...
    def _forward(self, inputs):
        ...
        agents_emb = (agents_emb + self.agents_positional_embedding[:, :,:num_agents] + self.temporal_positional_embedding).view(B, -1, self.d_k)
    ...
    def forward(self, batch, eval = False):
        ...
        if eval:
            ...
            ground_truth = torch.cat([inputs['center_gt_trajs'][..., :2], inputs['center_gt_trajs_mask'].unsqueeze(-1)],
                                    dim=-1)
            loss = self.criterion(output, ground_truth, None)
            output['predicted_probability'] = F.softmax(output['predicted_probability'], dim=-1)
            return output, loss
