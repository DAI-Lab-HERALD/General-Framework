...
from Autobot.models.context_encoders import MapEncoderPtsMA
...
class AutoBotJoint(nn.Module):
    ...
    def __init__(self, config):
        ...
        self._M = 0 # num agents other then the main agent.
        ...
        self.use_map_lanes = config['use_map']
        self.predict_yaw = config['predict_yaw']
        self.num_agent_types = config['num_agent_types']
        ...
        # ============================== Positional encoder ==============================
        self.pos_encoder = PositionalEncoding(self.d_k, dropout=0.0, max_len = config['past_len'])
    ...
    def temporal_attn_fn(self, agents_emb, agent_masks, layer):
        ...
        pos_encoding = self.pos_encoder(agents_emb.reshape(T_obs, B * (self._M + 1), -1))
        agents_temp_emb = layer(pos_encoding, src_key_padding_mask=agent_masks)
    ...
    def forward(self, ego_in, agents_in, roads, agent_types):
        ...
        if self.use_map_lanes:
            orig_map_features, orig_road_segs_masks = self.map_encoder(roads.repeat_interleave(self._M + 1, dim = 1), agents_emb)
        ...
        # embed agent types
        agent_types_features = self.emb_agent_types(agent_types).unsqueeze(1).repeat(1, self.c, 1, 1).view(-1, self._M+1, self.d_k)
        agent_types_features = agent_types_features.unsqueeze(0).repeat(self.T, 1, 1, 1)

        # AutoBot-Joint Decoding
        assert self.Q.isfinite().all(), 'Decoding NaNs'
        ...
        assert out_dists.isfinite().all(), 'Output NaNs'
    ...
    def configure_optimizers(self):
        optimizer = optim.Adam(self.parameters(), lr=self.config['learning_rate'], eps=self.config['adam_epsilon'])
