...
class ECE_traj_timestep(evaluation_template):
    r'''
    ...
    .. math::
        F = {1\over{201}} \sum\limits_{k = 0}^{200} \left| \left({1\over{\sum\limits_{i = 1}^{N_{samples}} N_{agents, i}}} \left| \left\{i,j,t \, | \, m_{i,j,t} > {k\over{200}}  \right\} \right| \right) + {k\over{200}} - 1 \right|
    
    Here, :math:`m_{i,j,t}` is calculated for each sample :math:`i \in \{1, ..., N_{samples}\}`, agent :math:`j`, and timestep :math:`t \in T_{O,i}` with
    
    .. math::
        m_{i,j,t} = {1\over{|P|}} \left|\left\{ p \in P \, | \, L_{pred,i,p,j,t} > L_{i,j,t} \right\} \right|,
        
    where the true and predicted likelihoods for each prediction :math:`p \in P`
    
    .. math::
        & L_{i,j,t} & = P_{KDE,i,j,t} \left(\{x_{i,j} (t), y_{i,j} (t) \} \right) \\
        & L_{pred,i,p,j,t} & = P_{KDE,i,j,t} \left(\{x_{pred,i,p,j,t} (t), y_{pred,i,p,j} (t) \} \right)
    
    
    are calculated using a sample and agent specific gaussian Kernel Density Estimate :math:`P_{KDE,i,j,t}` trained on 
    all predictions (:math:`p \in P`) for sample :math:`i \in \{1, ..., N_{samples}\}`, agent :math:`j`, and timestep :math:`t \in T_{O,i}`
    ...
    def get_name(self = None):
        ...
        names = {'print': 'ECE (Trajectories, independent agents and timesteps prediction' + P_p + ')',
                 'file': 'ECE_traj_timestep' + P_f,
                 'latex': r'\emph{ECE$_{' + P_l + r'\text{Traj}, timestep}$}'}
        return names
