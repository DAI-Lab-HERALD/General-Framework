...
class JSD_traj_timestep(evaluation_template):
    r'''
    ...
    .. math::
        ...
        D_{KL,s} & = {1 \over{|S_s|}} \sum\limits_{(i, j, t) \in S_s} \ln 
                    \left({ P_{KDE,s} \left(\{x_{i,j} (t), y_{i,j} (t)\} \right)   
                           \over{{1 \over {2}} \left( P_{KDE,pred,s} \left(\{x_{i,j} (t), y_{i,j} (t)\} \right) 
                                                     + P_{KDE,s} \left(\{x_{i,j} (t), y_{i,j} (t)\} \right)
                                                     \right)}} \right) \\
        D_{KL,s, pred} & = {1 \over{|S_s| |P|}} \sum\limits_{(i, j, t) \in S_s}\sum\limits_{p \in P} \ln 
                    \left({ P_{KDE,pred,s} \left(\{x_{pred,i,p,j} (t), y_{pred,i,p,j} (t)\} \right)   
                           \over{{1 \over {2}} \left( P_{KDE,pred,s} \left(\{x_{pred,i,p,j} (t), y_{pred,i,p,j} (t)\} \right) 
                                                     + P_{KDE,s} \left(\{x_{pred,i,p,j} (t), y_{pred,i,p,j} (t)\} \right)
                                                     \right)}} \right)
    ...
    while :math:`P_{KDE,pred,s}` is trained on all predictions (:math:`p \in P`) for all predicted agents (:math:`(i, j, t) \in S_s`):
    ...
    def evaluate_prediction_method(self):
        ...
        KDE_pred_log_prob_true, KDE_pred_log_prob_pred = self.get_KDE_probabilities(joint_timesteps=False, joint_agents = False)
        KDE_true_log_prob_true, KDE_true_log_prob_pred = self.get_true_likelihood(joint_timesteps=False, joint_agents = False)
    ...
    def get_name(self = None):
        ...
        names = {'print': 'JSD (Trajectories, independent agent and timestep predictions' + P_p + ')',
                 'file': 'JSD_traj_timestep' + P_f,
                 'latex': r'\emph{JSD$_{' + P_l + r'\text{Traj}, timestep}$}'}
        return names
