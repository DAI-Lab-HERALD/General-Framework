...
class minFDE20_joint(evaluation_template):
    r'''
    The value :math:`F` of the minimum Final Displacement Error (assuming :math:`N_{agents}` jointly predicted agents :math:`j`), is calculated in the following way:
        
    .. math::
        F = {1 \over{N_{samples} }} \sum\limits_{i = 1}^{N_{samples}}  \underset{p \in P_{20}}{\min} 
            \left( \sqrt{{1\over{N_{agents}}} \sum\limits_{j = 1}^{N_{agents}} 
            \left( x_{i,j}(\max T_O) - x_{pred,i,p,j} (\max T_O) \right)^2 + \left( y_{i,j}(\max T_O) - y_{pred,i,p,j} (\max T_O) \right)^2} \right)
    ...
    def get_name(self = None):
        names = {'print': 'min FDE (20 samples, joint prediction)',
                 'file': 'minFDE20_joint',
                 'latex': r'\emph{min FDE$_{20, joint}$ [m]}'}
        return names
