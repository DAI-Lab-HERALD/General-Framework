...
| [most likely ADE (marginal)](https://github.com/DAI-Lab-HERALD/General-Framework/blob/main/Framework/Evaluation_metrics/ADE_ML_indep.py) | Trajectories | $`{1 \over{\sum\limits_{i = 1}^{N_{S}} N_{A, i}}} \sum\limits_{i = 1}^{N_{S}}  \sum\limits_{j = 1}^{N_{A,i}} {1\over{N_{O,i} }} \sum\limits_{t \in T_{O,i}} \sqrt{\left( x_{i,j}(t) - \hat{x}_{i,p^*_{i,j},j} (t) \right)^2 + \left( y_{i,j}(t) - \hat{y}_{,i,p^*_{i,j},j} (t) \right)^2} `$ |
