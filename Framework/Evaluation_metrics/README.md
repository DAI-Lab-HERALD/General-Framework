...
| [ECE](https://github.com/DAI-Lab-HERALD/General-Framework/blob/main/Framework/Evaluation_metrics/ECE_class.py) | Classifications | $`{1 \over{ N_{S} N_{B}}} \sum\limits_{i = 1}^{N_{S}}  \sum\limits_{k} \lvert \tilde{p}_{i,k} - \hat{p}_{i,k} \rvert  `$, with $` \tilde{p}_{i,k} = {\sum\limits_{b \in B} L_{b} \overline{p}_{i,k, b} \over {\sum\limits_{b \in B} L_{b}}} `$ |
