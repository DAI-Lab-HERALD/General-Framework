...
| [Miss rate (joint)](https://github.com/DAI-Lab-HERALD/General-Framework/blob/main/Framework/Evaluation_metrics/Miss_rate_joint.py) | Trajectories | $`{1\over{N_{S}}} \sum\limits_{i = 1}^{N_S} \underset{j \in \{1,\hdots, N_{A,i} \}}{\max} \begin{cases} 1 & \sqrt{\left( x_{i,j}(\max T_{O,i}) - x_{pred,i,p,j} (\max T_{O,i}) \right)^2 + \left( y_{i,j}(\max T_{O,i}) - y_{pred,i,p,j} (\max T_{O,i}) \right)^2} > 2 \forall p \in P \\ 0 & \text{otherwise} \end{cases}   `$ |
