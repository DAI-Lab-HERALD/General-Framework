...
| [NLL (joint)](https://github.com/DAI-Lab-HERALD/General-Framework/blob/main/Framework/Evaluation_metrics/KDE_NLL_joint.py) | Trajectories | $`- {1 \over{\sum\limits_{i = 1}^{N_{samples}} N_{agents, i}}}  \sum\limits_{i = 1}^{N_{samples}} \ln \left( P_{KDE,i} \left(\{\{\{x_{i,j} (t), y_{i,j} (t) \} \, \vert \; \forall\, t \in T_{O,i}\} \, \vert \; \forall \, j \} \right)\right) `$
