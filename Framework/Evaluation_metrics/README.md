...
| [ADE (joint)](https://github.com/DAI-Lab-HERALD/General-Framework/blob/main/Framework/Evaluation_metrics/ADE_joint.py) | Trajectories | $${1\over{\vert P_{N_p} \vert N_{S}}} \sum\limits_{i = 1}^{N_S}{1\over{N_{O,i} }}\sum\limits_{p \in P_{N_p}} \sum\limits_{t \in T_{O,i}} \sqrt{{1\over{N_{A,i}}} \sum\limits_{j = 1}^{N_{A,i}} D_{i,p,j}(t)}$$ |
| [min ADE (marginal)](https://github.com/DAI-Lab-HERALD/General-Framework/blob/main/Framework/Evaluation_metrics/minADE_indep.py) | Trajectories | $${1\over{\sum\limits_{i = 1}^{N_S} N_{A, i}}} \sum\limits_{i = 1}^{N_S}{1\over{N_{O,i} }}\sum\limits_{j = 1}^{N_{A,i}} \underset{p \in P_{N_p}}{\min} \; \sum\limits_{t \in T_{O,i}}  \sqrt{D_{i,p,j}(t)}$$ |
| [min ADE (joint)](https://github.com/DAI-Lab-HERALD/General-Framework/blob/main/Framework/Evaluation_metrics/minADE_joint.py) | Trajectories | $${1\over{N_{S}}} \sum\limits_{i = 1}^{N_S}{1\over{N_{O,i} }}\underset{p \in P_{N_p}}{\min} \;  \sum\limits_{t \in T_{O,i}} \sqrt{{1\over{N_{A,i}}} \sum\limits_{j = 1}^{N_{A,i}} D_{i,p,j}(t)}$$ |
| [most likely ADE (marginal)](https://github.com/DAI-Lab-HERALD/General-Framework/blob/main/Framework/Evaluation_metrics/ADE_ML_indep.py) | Trajectories | $${1 \over{\sum\limits_{i = 1}^{N_{S}} N_{A, i}}} \sum\limits_{i = 1}^{N_{S}} {1\over{N_{O,i} }} \sum\limits_{j = 1}^{N_{A,i}}  \sum\limits_{t \in T_{O,i}} \sqrt{D_{i,p^*_{i,j},j}(t)} \;,\;\; \text{with} \;p^*_{i,j} = \underset{p \in P_{N_p}}{arg \min} \; \hat{L}_{i,p,j}$$ |
| [most likely ADE (joint)](https://github.com/DAI-Lab-HERALD/General-Framework/blob/main/Framework/Evaluation_metrics/ADE_ML_joint.py) | Trajectories | $${1\over{ N_{S}}} \sum\limits_{i = 1}^{N_S}{1\over{N_{O,i} }} \sum\limits_{t \in T_{O,i}} \sqrt{{1\over{N_{A,i}}} \sum\limits_{j = 1}^{N_{A,i}} D_{i,p^*_{i},j}(t)}\;,\;\; \text{with} \;p^*_{i} = \underset{p \in P_{N_p}}{\text{arg} \min} \; \hat{L}_{i,p}$$ |
| [FDE (marginal)](https://github.com/DAI-Lab-HERALD/General-Framework/blob/main/Framework/Evaluation_metrics/FDE_indep.py) | Trajectories | $${1\over{\vert P_{N_p} \vert\sum\limits_{i = 1}^{N_S} N_{A, i}}} \sum\limits_{i = 1}^{N_S}\sum\limits_{p \in P_{N_p}}  \sum\limits_{j = 1}^{N_{A,i}} \sqrt{D_{i,p,j}(\max T_{O,i})}$$ |
| [FDE (joint)](https://github.com/DAI-Lab-HERALD/General-Framework/blob/main/Framework/Evaluation_metrics/FDE_joint.py) | Trajectories | $${1\over{\vert P_{N_p} \vert N_{S}}} \sum\limits_{i = 1}^{N_S}\sum\limits_{p \in P_{N_p}}  \sqrt{{1\over{N_{A,i}}} \sum\limits_{j = 1}^{N_{A,i}} D_{i,p,j}(\max T_{O,i})}$$ |
| [min FDE (marginal)](https://github.com/DAI-Lab-HERALD/General-Framework/blob/main/Framework/Evaluation_metrics/minFDE_indep.py) | Trajectories | $${1\over{\sum\limits_{i = 1}^{N_S} N_{A, i}}} \sum\limits_{i = 1}^{N_S} \sum\limits_{j = 1}^{N_{A,i}} \underset{p \in P_{N_p}}{\min} \; \sqrt{D_{i,p,j}(\max T_{O,i})}$$ |
| [min FDE (joint)](https://github.com/DAI-Lab-HERALD/General-Framework/blob/main/Framework/Evaluation_metrics/minFDE_joint.py) | Trajectories | $${1\over{N_{S}}} \sum\limits_{i = 1}^{N_S}\underset{p \in P_{N_p}}{\min} \;   \sqrt{{1\over{N_{A,i}}} \sum\limits_{j = 1}^{N_{A,i}} D_{i,p,j}(\max T_{O,i})}$$ |
| [Brier min FDE (marginal)](https://github.com/DAI-Lab-HERALD/General-Framework/blob/main/Framework/Evaluation_metrics/Brier_minFDE_indep.py) | Trajectories | $${1\over{\sum\limits_{i = 1}^{N_S} N_{A, i}}} \sum\limits_{i = 1}^{N_S} \sum\limits_{j = 1}^{N_{A,i}} \left(1 - {\exp(\hat{L}_{i,p^*_{i,j},j})\over{\sum\limits_{p = 1}^{\vert P_{N_p} \vert} \exp(\hat{L}_{i,p,j})}}   \right)^2 \sqrt{D_{i,p^*_{i,j},j}(\max T_{O,i})}\;,\;\; \text{with} \;p^*_{i,j} = \underset{p \in P_{N_p}}{arg \min} \; D_{i,p,j}(\max T_{O,i})$$ |
| [Brier min FDE (joint)](https://github.com/DAI-Lab-HERALD/General-Framework/blob/main/Framework/Evaluation_metrics/Brier_minFDE_joint.py) | Trajectories | $${1\over{N_{S}}} \sum\limits_{i = 1}^{N_S} \left(1 - {\exp(\hat{L}_{i,p^*_{i}})\over{\sum\limits_{p = 1}^{\vert P_{N_p} \vert} \exp(\hat{L}_{i,p})}}   \right)^2\sqrt{{1\over{N_{A,i}}} \sum\limits_{j = 1}^{N_{A,i}} D_{i,p^*_{i},j}(\max T_{O,i})}\;,\;\; \text{with} \;p^*_{i} = \underset{p \in P_{N_p}}{\text{arg} \min} \; {1\over{N_{A,i}}} \sum\limits_{j = 1}^{N_{A,i}} D_{i,p,j}(\max T_{O,i})$$ |
| [most likely FDE (marginal)](https://github.com/DAI-Lab-HERALD/General-Framework/blob/main/Framework/Evaluation_metrics/FDE_ML_indep.py) | Trajectories | $${1 \over{\sum\limits_{i = 1}^{N_{S}} N_{A, i}}} \sum\limits_{i = 1}^{N_{S}}  \sum\limits_{j = 1}^{N_{A,i}}   \sqrt{D_{i,p^*_{i,j},j}(\max T_{O,i})} \;,\;\; \text{with} \;p^*_{i,j} = \underset{p \in P_{N_p}}{arg \min} \; \hat{L}_{i,p,j}$$ |
| [most likely FDE (joint)](https://github.com/DAI-Lab-HERALD/General-Framework/blob/main/Framework/Evaluation_metrics/FDE_ML_joint.py) | Trajectories | $${1\over{ N_{S}}} \sum\limits_{i = 1}^{N_S}  \sqrt{{1\over{N_{A,i}}} \sum\limits_{j = 1}^{N_{A,i}} D_{i,p^*_{i},j}(\max T_{O,i})}\;,\;\; \text{with} \;p^*_{i} = \underset{p \in P_{N_p}}{\text{arg} \min} \; \hat{L}_{i,p}$$ |
| [NLL (marginal)](https://github.com/DAI-Lab-HERALD/General-Framework/blob/main/Framework/Evaluation_metrics/KDE_NLL_indep.py) | Trajectories | $$- {1 \over{\sum\limits_{i = 1}^{N_{S}} N_{A, i}}}  \sum\limits_{i = 1}^{N_{S}}  \sum\limits_{j = 1}^{N_{A,i}} L_{i,j} $$ |
| [NLL (joint)](https://github.com/DAI-Lab-HERALD/General-Framework/blob/main/Framework/Evaluation_metrics/KDE_NLL_joint.py) | Trajectories | $$- {1 \over{N_{S}}}  \sum\limits_{i = 1}^{N_{S}} L_{i}$$ |
| [ECE (marginal)](https://github.com/DAI-Lab-HERALD/General-Framework/blob/main/Framework/Evaluation_metrics/ECE_traj_indep.py) | Trajectories | $${1\over{201}} \sum\limits_{k = 0}^{200} \left\vert \left({1\over{\sum\limits_{i = 1}^{N_{S}} N_{A, i}}} \left\vert \left\{i,j \, \vert \, {1\over{\vert P_{N_p}\vert}} \left\vert \left\{ p \in P_{N_p} \, \vert \, \hat{L}_{i,p,j} > L_{i,j}\right\} \right\vert > {k\over{200}}  \right\} \right\vert \right) + {k\over{200}} - 1 \right\vert $$ |
| [ECE (joint)](https://github.com/DAI-Lab-HERALD/General-Framework/blob/main/Framework/Evaluation_metrics/ECE_traj_joint.py) | Trajectories | $${1\over{201}} \sum\limits_{k = 0}^{200} \left\vert \left({1\over{N_{S}}} \left\vert \left\{i \, \vert \, {1\over{\vert P_{N_p}\vert}} \left\vert \left\{ p \in P_{N_p} \, \vert \, \hat{L}_{i,p} > L_{i}\right\} \right\vert > {k\over{200}}  \right\} \right\vert \right) + {k\over{200}} - 1 \right\vert $$ |
| [Miss rate (marginal)](https://github.com/DAI-Lab-HERALD/General-Framework/blob/main/Framework/Evaluation_metrics/Miss_rate_indep.py) | Trajectories | $${1\over{\sum\limits_{i = 1}^{N_S} N_{A, i}}} \sum\limits_{i = 1}^{N_S} \sum\limits_{j = 1}^{N_{A,i}} \begin{cases} 1 & \sqrt{D_{i,p,j}(\max T_{O,i})} > 2 \forall p \in P_{N_p} \\ 0 & \text{otherwise} \end{cases}   $$ |
| [Miss rate (joint)](https://github.com/DAI-Lab-HERALD/General-Framework/blob/main/Framework/Evaluation_metrics/Miss_rate_joint.py) | Trajectories | $${1\over{N_{S}}} \sum\limits_{i = 1}^{N_S} \underset{j \in \{1,..., N_{A,i} \}}{\max} \begin{cases} 1 & \sqrt{D_{i,p,j}(\max T_{O,i})} > 2 \forall p \in P_{N_p} \\ 0 & \text{otherwise} \end{cases}   $$ |
...
- For trajectory based metrics, the following notation is used:
  ...
  - $P$: Set of indicators to different, randomly predicted trajectories (with $P=$ **self.data_set.num_samples_path_pred**).
  - $P_{N_p}$: A random subset of $P$ with $|P_{N_p}| = N_p$. In the respective metrics, $N_p$ is set by using *'num_preds' = <*$N_p$*>* when defining the respective [kwargs dictionary in the *simulations.py* file](https://github.com/DAI-Lab-HERALD/General-Framework/tree/main/Framework#metrics).
