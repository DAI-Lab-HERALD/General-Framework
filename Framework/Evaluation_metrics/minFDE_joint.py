...
class minFDE_joint(evaluation_template):
    r'''
    The value :math:`F` of the minimum Final Displacement Error (assuming :math:`N_{agents,i}` jointly predicted agents :math:`j`), is calculated in the following way:
        
    .. math::
        F = {1 \over{N_{samples} }} \sum\limits_{i = 1}^{N_{samples}}  \underset{p \in P}{\min} 
            \left( \sqrt{{1\over{N_{agents,i}}} \sum\limits_{j = 1}^{N_{agents,i}} 
    ...
    def get_name(self = None):        
        ...
        names = {'print': 'min FDE (' + N_p + 'joint prediction' + P_p + ')',
                'file': 'minFDE' + N_f + '_joint' + P_f,
                'latex': r'\emph{min FDE$_{' + N_l + P_l + r'joint}$ [m]}'}
        return names
