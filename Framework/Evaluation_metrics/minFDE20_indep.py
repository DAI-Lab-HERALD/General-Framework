...
class minFDE20_indep(evaluation_template):
    r'''
    The value :math:`F` of the minimum Final Displacement Error (assuming :math:`N_{agents}` independent agents :math:`j`), is calculated in the following way:
        
    .. math::
        F = {1 \over{N_{samples} N_{agents}}} \sum\limits_{i = 1}^{N_{samples}} \sum\limits_{j = 1}^{N_{agents}}  
            \underset{p \in P_{20}}{\min} \left( 
            \sqrt{\left( x_{i,j}(\max T_O) - x_{pred,i,p,j} (\max T_O) \right)^2 + \left( y_{i,j}(\max T_O) - y_{pred,i,p,j} (\max T_O) \right)^2} \right)
