...
| [Forking Paths](https://github.com/DAI-Lab-HERALD/General-Framework/blob/main/Framework/Data_sets/Forking_Paths.py) | This contains a subset of the ETH/UCY dataset, with the exception that at certain points in time 4 - 12 possible future behaviors are presented, generated by humans in a simulator with different goals. | None |
| [Forking Paths - augmented](https://github.com/DAI-Lab-HERALD/General-Framework/blob/main/Framework/Data_sets/Forking_Paths_augmented.py) | This an expansion of the Forking Paths dataset, where for each of the differing futures, 100 randomly adjusted variants were created (with careful avoidance of potential collisions). | None |
| [HighD](https://github.com/DAI-Lab-HERALD/General-Framework/blob/main/Framework/Data_sets/HighD_interactive.py) | This is a relatively small drone captured datasets of vehicle behavior on a number of different German highways. | None |
| [HighD - Direction](https://github.com/DAI-Lab-HERALD/General-Framework/blob/main/Framework/Data_sets/HighD_direction.py) | This dataset is HighD with the addition of general behavior classification. | Maneuver (left/right/straight) |
| [HighD - Lane change](https://github.com/DAI-Lab-HERALD/General-Framework/blob/main/Framework/Data_sets/HighD_lane_change.py) | This dataset is a subset of HighD, where only scenarios with lane changes (actual or potential) into the faster lane are included. | Gap acceptance (accept/reject) |
| [InD](https://github.com/DAI-Lab-HERALD/General-Framework/blob/main/Framework/Data_sets/InD_direction.py) | This is a subset of InD, which is a drone captured dataset of vehicle/pedestrian behavior at German unsignalled intersections. This subset focuses on the prediction of incoming vehicles, given their options of turning or going straight. | Chosen direction (stay, left, straight, right) |
| [INTERACTION](https://github.com/DAI-Lab-HERALD/General-Framework/blob/main/Framework/Data_sets/Interaction_interactive.py) | This is a large dataset collected in four interactive driving scenarios (merging and lane changing, roundabout, un-signalized intersection, and signalized intersection). | None |
| [Lyft - Level 5](https://github.com/DAI-Lab-HERALD/General-Framework/blob/main/Framework/Data_sets/Lyft_interactive.py) | This is a large dataset collected by a vehicle driving through urban Palo Alto (California). | None |
| [NuScenes](https://github.com/DAI-Lab-HERALD/General-Framework/blob/main/Framework/Data_sets/NuScenes_interactive.py) | This is a medium sized dataset collected by a vehicle driving through urban Boston (Massachusetts) and Singapore. | None |
| [RounD - Roundabout entry](https://github.com/DAI-Lab-HERALD/General-Framework/blob/main/Framework/Data_sets/RounD_round_about.py) | This is a relatively small drone captured dataset of mainly vehicle behavior at German roundabouts. It focuses on the entry behavior of vehicles, and their decision to enter or wait for vehicles inside the roundabout to pass. | Gap acceptance (accept/reject) |
| [Waymo](https://github.com/DAI-Lab-HERALD/General-Framework/blob/main/Framework/Data_sets/Waymo_interactive.py) | This is a large dataset collected by a vehicle driving through six different American cities. | None |
