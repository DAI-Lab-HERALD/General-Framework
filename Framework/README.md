...
- 'repetition': This gives instruction for the specific test set to be used.  This is based on the assumption that for a given splitting method, the whole dataset can be split preemptively into a number of orederd parts. A single repetition, from which one single training/testing split springs forth, can be given in three different forms:
  - As an integer (*int*), to give the place in the ordered list of the presplit part to be used as a training set.
  - As an string (*str*), to refer to a specific name in cases that each presplit part can be assigned by [such a name](https://github.com/julianschumann/General-Framework/blob/main/Framework/Splitting_methods/README.md#checking-applicability) (such as in the case of splitting by location).
  - As an tuple (*tuple*), where each entry is either a str or an integer. In this case, one will select the presplit part to the aforementioned method and use their union as the test set.
